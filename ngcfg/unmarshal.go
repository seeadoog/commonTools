...
func setInto(e *Elem,val reflect.Value)error{
	switch val.Kind() {
	case reflect.Ptr:
		if val.IsNil(){
	...
	case reflect.Map:
		...
		for item !=nil{
			...
			case *Elem:
				...
				if err:=setInto(mv.(*Elem),typv);err != nil{
					return err
				}
				if mvType.Kind() == reflect.Ptr{
	...
	case reflect.Slice:
		...
		for item != nil{
			...
			case []string:
				if err:=setVal(item.Val.([]string),itemVal);err != nil{
					return err
				}
			case *Elem:
				if err:=setInto(item.Val.(*Elem),itemVal);err != nil{
					return err
				}
			}
			if sliceElemType.Kind() == reflect.Ptr{
